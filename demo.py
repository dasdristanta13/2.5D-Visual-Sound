...
def main():
    ...
    with open("/home/sysadm/Documents/Dristanta_ML_Project/Splits/split3.json", 'r') as fd:
        split = json.load(fd)

    song = [song.split("/")[-1] for song in split["test"]]
    print(song)
    for a in tqdm(song):
        path = os.path.join(opt.input_audio_path, a)

        # load the audio to perform separation
        audio, audio_rate = librosa.load(path, sr=opt.audio_sampling_rate, mono=False)
        ...
        # perform spatialization over the whole spectrogram in a sliding-window fashion
        ...
        while sliding_window_start + samples_per_window < audio.shape[-1]:
            ...
            data['audio_diff_spec'] = torch.FloatTensor(
                generate_spectrogram(audio_segment_channel1 - audio_segment_channel2)).unsqueeze(
                0)  # unsqueeze to add a batch dimension
            data['audio_mix_spec'] = torch.FloatTensor(
                generate_spectrogram(audio_segment_channel1 + audio_segment_channel2)).unsqueeze(
                0)  # unsqueeze to add a batch dimension
            # get the frame index for current window
            frame_index = int(round((((sliding_window_start + samples_per_window / 2.0) / audio.shape[
                -1]) * opt.input_audio_length + 0.05) * 10))
            image = Image.open(os.path.join(opt.video_frame_path,
                                            str(a.split(".")[0]) + str(".mp4/") + str(frame_index).zfill(
                                                6) + '.png')).convert('RGB')
            ...
            reconstructed_signal_diff = librosa.istft(reconstructed_stft_diff, hop_length=160, win_length=400,
                                                      center=True,
            ...
            reconstructed_binaural = np.concatenate(
                (np.expand_dims(reconstructed_signal_left, axis=0), np.expand_dims(reconstructed_signal_right, axis=0)),
                axis=0) * normalizer

            binaural_audio[:, sliding_window_start:sliding_window_end] = binaural_audio[:,
                                                                         sliding_window_start:sliding_window_end] + reconstructed_binaural
        ...
        # ISTFT to convert back to audio
        reconstructed_stft_diff = predicted_spectrogram[0, :, :] + (1j * predicted_spectrogram[1, :, :])
        reconstructed_signal_diff = librosa.istft(reconstructed_stft_diff, hop_length=160, win_length=400, center=True,
                                                  length=samples_per_window)
        reconstructed_signal_left = (audio_segment_mix + reconstructed_signal_diff) / 2
        reconstructed_signal_right = (audio_segment_mix - reconstructed_signal_diff) / 2
        reconstructed_binaural = np.concatenate(
        ...
        curr_output_dir_root = os.path.join(opt.output_dir_root, str(a.split(".")[0]) + str(".mp4/"))
